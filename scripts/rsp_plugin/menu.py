...
def register():
    ...
    Metashape.app.addMenuItem(SCALING_LABEL, scaling_flow.run)

    _remove_if_present(FILTER_LABEL)
    Metashape.app.addMenuItem(FILTER_LABEL, scaling_flow.run_filter_only)

    Metashape.app.addMenuSeparator(SEPARATOR_LABEL)

    _remove_if_present(CALIBRATION_LABEL)
    Metashape.app.addMenuItem(CALIBRATION_LABEL, calibration_flow.run)
